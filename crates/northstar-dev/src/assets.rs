//! `northstar-dev assets` — the `.nspkg` tooling absorbed from the original
//! standalone `northstar-cli`: `classify`, `inspect`, `pack-test`,
//! `unpack-test`.
//!
//! Deliberately over `northstar-core` only — no Bevy dependency here, see
//! the workspace `AGENTS.md`. The pack/unpack commands only prove lossless
//! opaque-chunk round-tripping; they are not the eventual mod-authoring
//! interface.
...
use crate::{one_arg, two_args};

pub fn run(args: Vec<String>) -> Result<(), String> {
    let mut args = args.into_iter();
    let Some(subcommand) = args.next() else {
        print_usage();
        return Err("missing assets subcommand".to_string());
    };
    ...
    match subcommand.as_str() {
        ...
        other => Err(format!(
            "unknown assets subcommand \"{other}\"; see `northstar-dev assets help`"
        )),
    }
}
...
fn print_usage() {
    eprintln!(
        "northstar-dev assets — .nspkg developer tooling\n\
         \n\
         USAGE:\n\
         \x20   assets classify <path>                        classify by filename only, no file I/O\n\
         \x20   assets inspect <path>                          print an .nspkg container's header and chunk index\n\
         \x20   assets pack-test <output-path>                 write a small deterministic test container\n\
         \x20   assets unpack-test <input-path> <output-dir>    extract every chunk's raw bytes to files"
    );
}

/// `assets classify <path>` — filename-only, performs no file I/O, need
/// not exist.
fn run_classify(args: Vec<String>) -> Result<(), String> {
    let path = one_arg(args, "usage: assets classify <path>")?;
...
/// `assets inspect <path>` — reads the file and prints the experimental
/// container's header and chunk index.
fn run_inspect(args: Vec<String>) -> Result<(), String> {
    let path = one_arg(args, "usage: assets inspect <path>")?;
...
/// `assets pack-test <output-path>` — writes a small, deterministic
/// two-chunk container, self-identified by classifying `output-path`'s own
/// filename.
fn run_pack_test(args: Vec<String>) -> Result<(), String> {
    let output_path = one_arg(args, "usage: assets pack-test <output-path>")?;

    let identity = ClassifiedFilename::classify_path(Path::new(&output_path))
        .map_err(|e| format!("{output_path}: {e}"))?;

    let mut writer =
        ContainerWriter::new(identity).with_metadata(*b"northstar-dev pack-test fixture");
    writer
        .push_chunk(
...
/// `assets unpack-test <input-path> <output-dir>` — extracts every chunk's
/// raw bytes (and the metadata payload) to files, proving lossless
/// round-trip.
fn run_unpack_test(args: Vec<String>) -> Result<(), String> {
    let (input_path, output_dir) = two_args(
        args,
        "usage: assets unpack-test <input-path> <output-directory>",
    )?;
